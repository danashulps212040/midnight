#!/usr/bin/env python3
"""
Script para testar alteração de custo de material (MySQL)
...
def verificar_material_atual():
    ...
    for material in materiais:
        print(f"  ID: {material['id']}, Nome: {material['nome']}, Código: {material['codigo']}, Categoria: {material['categoria_id']}")
    ...
    if ultima_entrada:
        print(f"Último custo registrado: R$ {ultima_entrada['custo_unitario']:.2f}")
        print(f"Data da última entrada: {ultima_entrada['data_entrada']}")
        print(f"ID da entrada: {ultima_entrada['id']}")
    else:
...
def verificar_deteccao_manual(material_id, produto_id=61):
    ...
    # Buscar último custo vs custo anterior usado no cálculo
    cursor.execute("""
    ...
    if len(entradas) >= 2:
        custo_atual = entradas[0]['custo_unitario']
        custo_anterior = entradas[1]['custo_unitario']
...
def main():
    ...
    print("TESTE DE ALTERAÇÃO DE CUSTO DE MATERIAL (MySQL)")
    print("=" * 55)
